...
public class StoreTests : AggregateTests<Store, StoreId>
{
    ...
    public void UpdateStoreShouldUpdateNameAndRaiseStoreNameUpdatedDomainEventWhenDifferentNameIsGiven()
    {
        ...
        Given(() => Store.Create(_ownerId, Name, Description, Address))
            .When(store => store.Update(newName, null, null))
            .Then(store => Assert.Equal(newName, store.Name))
            .Then<V1StoreNameUpdatedDomainEvent>(@event => Assert.Equal(newName, @event.Name));
    }

    [Fact]
    public void
        UpdateStoreShouldUpdateDescriptionAndRaiseStoreDescriptionUpdatedDomainEventWhenDifferentDescriptionIsGiven()
    {
        const string newDescription = "New Store Description";

        Given(() => Store.Create(_ownerId, Name, Description, Address))
            .When(store => store.Update(null, newDescription, null))
            .Then(store => Assert.Equal(newDescription, store.Description))
            .Then<V1StoreDescriptionUpdatedDomainEvent>(@event => Assert.Equal(newDescription, @event.Description));
    }
    ...
    public void RemoveProductFromStoreShouldRemoveAndRaiseProductRemovedFromStoreDomainEvent()
    {
        ...
        Given(() => store)
            ...
            .Then<V1ProductRemovedFromStoreDomainEvent>(@event => Assert.Equal(product, @event.Product));
    }
}
