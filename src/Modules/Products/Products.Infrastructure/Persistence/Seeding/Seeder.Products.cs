        ...
        int storeProductCountPerStore, CancellationToken cancellationToken)
    {
        ...
        for (var i = 0; i < storeIds.Count; i++)
        {
            for (var j = 0; j < storeProductCountPerStore; j++)
            {
                ...
                if (await dbContext.Products.SingleOrDefaultAsync(
                        sp => sp.StoreId == storeId && sp.ProductTemplateId == productTemplateId, cancellationToken)
                    is not Product storeProduct)
                {
#pragma warning disable CA5394
                    storeProduct = Product.Create(storeId, productTemplateId, name, "Seed", random.Next(1, 50),
