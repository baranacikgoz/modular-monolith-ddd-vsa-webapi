...
- Request handlers get their own receive endpoint policy through `InterModuleRequestHandlerDefinition<THandler>` (registered per handler by `Setup.MassTransit.cs`, which excludes them from the plain consumer scan): no retry, handler timeout = `InterModuleRequestOptions.TimeoutSeconds`, concurrency from `HandlerPrefetchCount`/`HandlerConcurrentMessageLimit`. The bus-level exponential retry is for event consumers only. Per-request caller timeouts: `InterModuleRequestOptions.Timeouts["XRequest"]`. A faulted or timed-out request surfaces to the HTTP client as 503 + `Retry-After` (`DependencyUnavailable`).
