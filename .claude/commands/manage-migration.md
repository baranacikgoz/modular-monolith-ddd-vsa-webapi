...
2. Review the generated files: unintended drops or renames, missing indexes. A change to shared EF config (for example the `AuditLog` base configuration) needs a migration in every module that shares it, or boot fails with `PendingModelChangesWarning`. A generated `STORED` tsvector column (full-text search, see `docs/full-text-search.md`) is recomputed for every existing row on creation, taking an `ACCESS EXCLUSIVE` lock; treat it as a maintenance-window operation on a large table. A plain `CREATE INDEX` on a table that already holds production data takes a write lock while it builds: create the index `CONCURRENTLY` instead, through `migrationBuilder.Sql(..., suppressTransaction: true)` (Postgres cannot run it inside a transaction block), and confirm with devops that the script runner does not wrap the file in one. On a table that is still empty a plain index is fine.
