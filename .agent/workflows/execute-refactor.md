...
2. **Pre-Flight Check**: Before changing any business logic, run `dotnet test` on the target module to ensure the existing tests provide a green baseline. If there are no tests for the legacy code, warn the user.
...
4. **Post-Flight Verification**: 
    - Re-run `dotnet test` on the affected module(s) to guarantee no regressions (Green state).
