...
3. **Integration Tests**: Write the Slice Integration Tests for the REPR Endpoints extending `BaseIntegrationTest` to automatically utilize `Testcontainers` and database `Respawn` cleanup.
4. **Confirm Red State**: Run `dotnet test src/Modules/[Module]/[Module].Tests` and inspect the output. You MUST confirm that the tests fail (due to unimplemented logic) to establish a valid baseline.
